...
package body GPIO is
   ...
   begin
      -- Activation du timer;
      PRR_Bits (PRTIM0_Bit) := False;
      TCCR0A_Bits := (WGM00_Bit  => True,
                      WGM01_Bit  => True,
                      others => False);
      TCCR0B_Bits := (CS02_Bit => True,
                      others => False);
      Timer0_Init := True;
   end Init_Timer0;
   ...
   begin
      -- Activation du timer;
      PRR_Bits (PRTIM1_Bit) := False;
      TCCR1A_Bits := (WGM11_Bit  => True,
                      others => False);
      ...
      ICR1 := 40000;  -- 50 Hz => 16000000/8/50 = 40000
      Timer1_Init := True;
   end Init_Timer1;
   ...
   begin
      -- Activation du timer;
      PRR_Bits (PRTIM2_Bit) := False;
      TCCR2A_Bits := (WGM20_Bit  => True,
                      WGM21_Bit  => True,
                      others => False);
      TCCR2B_Bits := (CS21_Bit => True,
                      CS22_Bit => True,
                      others => False);
      Timer2_Init := True;
   end Init_Timer2;
         ...
         case nomPort is
            when D3 => -- Timer 2
               ...
               TCCR2B_Bits := (WGM22_Bit => False,
                               ...
                               CS22_Bit => False,
                               others => False);
               TCCR2A_Bits(COM2A1_Bit) := True; -- Equivalent à mettre COM2A1 à 1
            when D5 => -- Timer 0
               TCCR0A_Bits := (WGM00_Bit  => True,
                               WGM01_Bit  => True,
                               others => False);
               TCCR0B_Bits := (WGM02_Bit => False,
                               ...
                               CS02_Bit => False,
                               others => False);
               TCCR0A_Bits(COM0A1_Bit) := True; -- Equivalent à mettre COM0A1 à 1
            when D6 => -- Timer 0
               TCCR0A_Bits := (WGM00_Bit  => True,
                               WGM01_Bit  => True,
                               others => False);
               TCCR0B_Bits := (WGM02_Bit => False,
                               ...
                               CS02_Bit => False,
                               others => False);
               TCCR0A_Bits(COM0B1_Bit) := True; -- Equivalent à mettre COM0B1 à 1
            when D9 => -- Timer 1
               -- Activation du timer;
               ...
               TCCR1B_Bits := (WGM12_Bit => True,
                               ...
                               CS12_Bit => False,
                               others => False);
               TCCR1A_Bits(COM1A1_Bit) := True; -- Equivalent à mettre COM1A1 à 1
            when D10 => -- Timer 1
               -- Activation du timer;
               MCU.PRR_Bits (MCU.PRTIM1_Bit) := False;
               TCCR1A_Bits := (WGM10_Bit  => True,
               ...
               TCCR1B_Bits := (WGM12_Bit => True,
                               ...
                               CS12_Bit => False,
                               others => False);
               TCCR1A_Bits(COM1B1_Bit) := True; -- Equivalent à mettre COM1B1 à 1
            when D11 => -- Timer 2
               TCCR2A_Bits := (WGM20_Bit  => True,
                               WGM21_Bit  => True,
                               others => False);
               TCCR2B_Bits := (WGM22_Bit => False,
                               ...
                               CS22_Bit => False,
                               others => False);
               TCCR2A_Bits(COM2B1_Bit) := True; -- Equivalent à mettre COM2B1 à 1
            when others => null;
         end case;
      elsif (modePort = PWMOutput) then
         case nomPort is
            when D3 => -- Timer 2
               ...
               TCCR2A_Bits(COM2B1_Bit) := True; -- Equivalent à mettre COM2B1 à 1
            when D5 => -- Timer 0
               if (Timer0_Init = False) then
                  Init_Timer0;
               end if;
               TCCR0A_Bits(COM0B1_Bit) := True; -- Equivalent à mettre COM0A1 à 1
            when D6 => -- Timer 0
               if (Timer0_Init = False) then
                  Init_Timer0;
               end if;
               TCCR0A_Bits(COM0A1_Bit) := True; -- Equivalent à mettre COM0B1 à 1
            when D9 => -- Timer 1
   ...
   procedure PWMWrite(nomPort : lvcPins; value : Unsigned_16) is
   begin
   -- For the setting of OCRx value value, there are two cases :
   -- - 16 bit timers => the value is an value *2 because of a precision of 0.5 µs
   -- - 8 bits timers => the value is divide by 16 because of frequency 16MHz
   --                    and the prescaler is 256. So the precision is 16 µs
      case nomPort is
         when D3 => -- OC2B
            MCU.OCR2B := To_Uint8(value / 16);
         when D5 => -- OC0B
            MCU.OCR0B := To_Uint8(value / 16);
         when D6 => -- OC0A
            MCU.OCR0A := To_Uint8(value / 16);
         when D9 => -- OC1A
            MCU.OCR1A := 2*value;
         when D10 => -- OC1B
            MCU.OCR1B := 2*value;
         when D11 => -- OC2A
            MCU.OCR2A := To_Uint8(value / 16);
         when others => null;
      end case;
   end PWMWrite;
end GPIO;
