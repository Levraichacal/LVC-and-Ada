...
package GPIO is
   ...
   type lvcMode is (Input,Output,AnalogOutput,PWMOutput);
   ...
   -- Procedure to write a PWM on a PWM port
   -- The frequeny is fixed to at 50 Hz for 16-bits Timers and 62.5Hz
   -- for 8-bits Timers (which is support for common servomotor)
   -- Common value used are between 800 and 2400
   -- It works on D3, D5, D6, D9, D10 and D11.
   procedure PWMWrite(nomPort : lvcPins; value : Unsigned_16);
