...
-- function U16_To_U64 is
   ...
   -- function U64_To_U16 is
      -- new Ada.Unchecked_Conversion (Source => Unsigned_64,
                                    -- Target => Unsigned_16);

   -- procedure LCD_Print(Message : Unsigned_64; base : Unsigned_8 := 10) is
      -- buf : String :="";
      -- n,m,a : Unsigned_64 := Message;
      -- b,c,d : Unsigned_16;
      -- baseTemp : Unsigned_16 := U8_To_U16(base) * U8_To_U16(base) * U8_To_U16(base);
      -- i : Integer;
   -- begin
      -- i:=0;
      -- while(n>0) loop
         -- m := n;
         -- n := m / U16_To_U64(baseTemp);
         -- a := m - U16_To_U64(baseTemp) * n;
         -- b := U64_To_U16(a);
         -- for j in 1..3 loop
            -- i := i + 1;
            -- c := b;
            -- b := c / U8_To_U16(base);
            -- d := c - U8_To_U16(base) * b;
            -- if (d<10) then
               -- buf(i) := Character'Val(d + Character'Pos('0'));
            -- else
               -- buf(i) := Character'Val(d + Character'Pos('A') - 10);
            -- end if;
         -- end loop;
      -- end loop;
      -- -- For print the 0
