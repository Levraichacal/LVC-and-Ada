...
package body LVC.PINS is
   ...
   function portOutputRegisterValue(port: lvcPort) return Unsigned_8 is
   begin
      case port is
         when PB =>
            return MCU.PORTB;
   ...
   function portInputRegisterValue(port: lvcPort) return Unsigned_8 is
      --temp : Bits_In_Byte := (others => False);
   begin
      case port is
         when PB =>
            return MCU.PINB;
   ...
   end portInputRegisterValue;

   function portModeRegister(port: lvcPort) return Unsigned_8 is
   begin
      case port is
         when PB =>
            return MCU.DDRB;
