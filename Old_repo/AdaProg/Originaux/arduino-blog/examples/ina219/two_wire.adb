...
package body Two_Wire is
   ...
   pragma Volatile (Data_Index);

   Data_Max : Buffer_Size;
   pragma Volatile (Data_Max);

   Data_Received : Buffer_Size;
   pragma Volatile (Data_Received);

   Data_Sent : Boolean; -- All data sent?
   pragma Volatile (Data_Sent);
   ...
   begin
      ...
      DDRC_Bits (4) := DD_Input;
      DDRC_Bits (5) := DD_Input;
      PORTC_Bits (4) := True;
      PORTC_Bits (5) := True;
   ...
   procedure Request_Data (Address : Interfaces.Unsigned_8;
                           Data    : out Data_Buffer) is
      use type Interfaces.Unsigned_8;

      Amount : Buffer_Size := Data'Last - Data'First + 1;
   begin
      Request_Data (Address, Amount);

      loop
         exit when Data_Received = Data_Max;
         if TWI_Error_State /= TWI_No_Error then
            return;
         end if;
      end loop;
      ...
      for I in Data'Range loop
         Data (I) := Buffer (Buffer'First + (I - Data'First));
      end loop;
   end Request_Data;
