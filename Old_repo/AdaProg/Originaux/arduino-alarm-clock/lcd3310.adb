...
package body LCD3310 is
   ...
   procedure Draw_Char (Ch : Character;
      ...
      Place : System.Address := Fonts'Address;
   begin
      for I in Unsigned_8 range 0 .. 4 loop
         if Character'Pos (Ch) < 32 or Character'Pos (Ch) > 93 then
            -- Place := Place + System.Address (I);
            -- Offset := Fonts'Address + System.Address (Place);
            Offset := Fonts (0, I);
         else
            Place := Place + System.Address
               ((Character'Pos(Ch) - 32)* Fonts_W + Unsigned_16 (I));
            Offset := Fonts (Character'Pos (Ch) - 32, I);
         end if;
