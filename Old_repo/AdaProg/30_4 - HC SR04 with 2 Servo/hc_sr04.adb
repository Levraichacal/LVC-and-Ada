...
package body HC_SR04 is
   ...
   procedure Set_Servo10(valeur_PWM : Interfaces.Unsigned_16) is
   begin
      MCU.OCR1B := valeur_PWM *2;
   end Set_Servo10;
   

   procedure Init_Servo10 is
   begin
      MCU.DDRB_Bits (2) := DD_Output; -- D10 en sortie
      MCU.TCCR1A_Bits(5) := True; -- Equivalent à mettre COM1B1 à 1
      Set_Servo10(1500); -- PWM à 1500 TODO fonction trim
   end Init_Servo10;
   ...
   begin
      ...
      Init_Servo9;  -- Initialisation du servo 9
      Init_Servo10; -- Initialisation du servo 10
   end Init_HC_SR04;
