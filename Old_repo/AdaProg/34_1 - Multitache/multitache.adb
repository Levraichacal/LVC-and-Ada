...
procedure Multitache is
 ...
 when Clear is   -- the entry guard - calling tasks are blocked until the condition is true
      begin
         Clear := False;
         AVR.UART.Put (Airplane_ID'Image (ID) & " on runway ");
         AVR.UART.CRLF;
      end;
 ...
 when not Clear is
      begin
         Clear := True;
         AVR.UART.Put (Airplane_ID'Image (ID) & " cleared runway ");
         AVR.UART.CRLF;
      end;
 ...
 when Clear is
   ...
   begin
      ...
      AVR.UART.Put (Airplane_ID'Image (ID) & "  taking off...");
      AVR.UART.CRLF;
      delay 2.0;
      ...
      loop
         ...
         or
            ...
            AVR.UART.Put (Airplane_ID'Image (ID) & "   in holding pattern");  -- simply print a message
            AVR.UART.CRLF;
         end select;
      end loop;
      delay 4.0;  -- do landing approach...
      AVR.UART.Put (Airplane_ID'Image (ID) & "            touched down!");
      AVR.UART.CRLF;
