...
--  | |     | |     | |     | |     | |     | |     | |     | |     | |     | |
--  ---     ---     ---     ---     ---     ---     ---     ---     ---     ---
--      CH1     CH2     CH3     CAM1    CAM2    CAM3    CAM4    CAM5    CAM6
...
--  LES VARIABLES
-----------------------------------------------------------------------------------------------------------
   servo_start : Tableau_Data := (0,0,0,0,0,0,0,0,0);
   servo_width : Tableau_Data := (1500,1500,1500,1500,1500,1500,1500,1500,1500);
...
-----------------------------------------------------------------------------------------------------------
   ...
   begin
      pinMode(D10, Input); -- CH1
      pinMode(D11, Input); -- CH2
      pinMode(D12, Input); -- CH3
   end init_PortB;

   procedure init_PortC is
   begin
      pinMode(A0, Input); -- CAM1
      pinMode(A1, Input); -- CAM2
      pinMode(A2, Input); -- CAM3
      pinMode(A3, Input); -- CAM4
      pinMode(A4, Input); -- CAM5
      pinMode(A5, Input); -- CAM6
   end init_PortC;
      ...
      case Pin_Mask is
         ...
         when 16#20# =>
            ...
            if (PINC_Stat(5)) then
               ...
               if(time >= servo_start(9)) then
                  servo_width(9) := (time - servo_start(9));
               else
                  servo_width(9) := (65535 - servo_start(9) + time);
               end if;
            end if;
            MCU.PCMSK1_Bits(MCU.PCINT13_Bit) := True;
...
-- Disable interrupts during reading value or register
   ...
   begin
      ...
      case Pin_Mask is
         -- Channel 1
         when 16#04# =>
            MCU.PCMSK0_Bits(MCU.PCINT2_Bit) := False;
            if (PINB_Stat(2)) then
               servo_start(7) := time;
            else
               if(time >= servo_start(7)) then
...
-- Stop interrupts during writing the register value!
   ...
   begin
      case valueSend is
         ...
         when 35 =>
            AVR.Interrupts.Disable;
            MCU.UDR0 := High_Byte (servo_width(9));
            AVR.Interrupts.Enable;

            valueSend := 36;
         when 36 =>
            AVR.Interrupts.Disable;
            MCU.UDR0 := Low_Byte (servo_width(9));
            AVR.Interrupts.Enable;
            valueSend := 1;
         when others =>
            null;
