...
package body I2c_ecran1602 is
   ...
   procedure LCD_Print(Message : Float; Number : Unsigned_8 := 2) is
      buf : String :="";
      num, round, remainder,temp : Float;
      n,m,a : Integer_16;
      --a : Unsigned_8;
      i : Integer;
   begin
      ...
      i:=0;
      num := num + round; -- For rounding correctly the number
      LCD_Print(Integer_16(num));
      --temp := Float'floor(num);--Integer_32(num); -- Partie entière?
      n := Integer_16(Integer(num));
      --LCD_Print(n);
      remainder := num - temp; -- La partie après la virgule
      if (Number>0) then
         LCD_Print(".");
      end if;
   end LCD_Print;
