...
--      MCU.SPSR_Bits(MCU.SPIF):= True := 0;
      ...
      digitalWrite(D10,High_lvl);
      MCU.SPDR := MAX7219_MODE_DECODE;
      -- wait for complete transmission
      while(not((MCU.SPSR and MCU.SPIF_Mask) = MCU.SPIF_Mask)) loop
         null;
      end loop;
      MCU.SPDR := 16#00#;
      -- wait for complete transmission
      ...
      digitalWrite(D10,Low_lvl);
      digitalWrite(D10,High_lvl);
      MCU.SPDR := MAX7219_MODE_SCAN_LIMIT;
      -- wait for complete transmission
      while(not((MCU.SPSR and MCU.SPIF_Mask) = MCU.SPIF_Mask)) loop
         null;
      end loop;
      ...
      digitalWrite(D10,Low_lvl);
      digitalWrite(D10,High_lvl);
      MCU.SPDR := MAX7219_MODE_INTENSITY;
      -- wait for complete transmission
      while(not((MCU.SPSR and MCU.SPIF_Mask) = MCU.SPIF_Mask)) loop
         null;
      end loop;
